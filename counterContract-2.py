...
def approval_program():
    ...
    le = 8
    scratchCount_list  = [ScratchVar(TealType.uint64) for x in range(le)]
    ...
    handle_noop = Seq([
        For(i.store(Int(0)), i.load() < Txn.application_args.length(), i.store( i.load()+Int(1) ) ).Do(
            Cond(
                [Txn.application_args[i.load()] == Bytes("Add"), add],
                [Txn.application_args[i.load()] == Bytes("Deduct"), deduct]
            )
        ),
        Return(Int(1))
    ])
